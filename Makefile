CXX=g++
CXXFLAGS=-g -Wall -std=c++11
# Uncomment for parser DEBUG
#DEFS=-DDEBUG

# Adding book.o, clothing.o, movie.o to the list of object files
OBJS=amazon.o mydatastore.o user.o db_parser.o product.o product_parser.o util.o book.o clothing.o movie.o

all: amazon

amazon: $(OBJS)
	$(CXX) $(CXXFLAGS) $(DEFS) -o $@ $(OBJS)

# Rule for compiling amazon.o
amazon.o: amazon.cpp db_parser.h datastore.h product_parser.h mydatastore.h
	$(CXX) $(CXXFLAGS) $(DEFS) -o $@ -c amazon.cpp

# Rule for compiling mydatastore.o
mydatastore.o: mydatastore.cpp mydatastore.h datastore.h product.h user.h
	$(CXX) $(CXXFLAGS) $(DEFS) -o $@ -c mydatastore.cpp

# Rule for compiling user.o
user.o: user.cpp user.h 
	$(CXX) $(CXXFLAGS) $(DEFS) -o $@ -c user.cpp

# Rule for compiling db_parser.o
db_parser.o: db_parser.cpp db_parser.h product.h product_parser.h user.h datastore.h mydatastore.h
	$(CXX) $(CXXFLAGS) $(DEFS) -o $@ -c db_parser.cpp

# Rule for compiling product.o
product.o: product.cpp product.h 
	$(CXX) $(CXXFLAGS) $(DEFS) -o $@ -c product.cpp

# Rule for compiling product_parser.o
product_parser.o: product_parser.cpp product_parser.h product.h 
	$(CXX) $(CXXFLAGS) $(DEFS) -o $@ -c product_parser.cpp

# Rule for compiling util.o
util.o: util.cpp util.h
	$(CXX) $(CXXFLAGS) $(DEFS) -o $@ -c util.cpp

# Rule for compiling book.o
book.o: book.cpp book.h product.h
	$(CXX) $(CXXFLAGS) $(DEFS) -o $@ -c book.cpp

# Rule for compiling clothing.o
clothing.o: clothing.cpp clothing.h product.h
	$(CXX) $(CXXFLAGS) $(DEFS) -o $@ -c clothing.cpp

# Rule for compiling movie.o
movie.o: movie.cpp movie.h product.h
	$(CXX) $(CXXFLAGS) $(DEFS) -o $@ -c movie.cpp

clean:
	rm -f *.o amazon
